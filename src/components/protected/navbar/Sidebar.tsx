          ...
          {dropdownNavData.map((item, i) => {
            return (
              <Link
                href={item.link}
                key={i}
                className={`p-2 rounded-xl border-2 border-white flex items-center ${!openSidebar ? " justify-center" : "w-[200px]"} ${paths[paths.length - 1] === item.link ? "bg-black/60 text-white" : "bg-main-bg"}`}
              >
                <Image
                  ...
                  width={24}
                  height={24}
                  className={`${paths[paths.length - 1] === item.link && "invert"}`}
                />
